...
class DataArguments:
    ...
    val_ratio: float = field(default=0.05, metadata={"help": "Ratio of training data used for validation."})
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    if data_args.val_ratio > 0:
        val_size = max(1, int(len(dataset) * data_args.val_ratio))
...
def train():
    ...
    callbacks = [EarlyStoppingCallback(early_stopping_patience=3)] if data_module["eval_dataset"] is not None else []
