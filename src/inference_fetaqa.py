...
def tokenize_data(to_tokenize_queue, to_output_queue, rank):
    model_name = '/data/zyx/2026/2D-TPE/output/fetaqa_2d_merged'
...
def output_data(to_output_queue):
    ...
    while True:
        ...
        if data is None:
            ...
            with open('/data/zyx/2026/2D-TPE/res/fetaqa_2d_res.json', 'a') as f:
                try:
                    json.dump(data, f)
                    f.write('\n')
                except:
...
if __name__ == "__main__":
    ...
    # start
    reader_process = multiprocessing.Process(target=read_data, args=("/data/zyx/dataset/TableInstruct/eval_data/in_domain_test/fetaqa_test.json", to_tokenize_queue))
